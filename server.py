...
def parse_saved_email_metadata(filepath):
    """
    Reads saved email and extracts:
    - sender
    - datetime string
    - title
    """
...
def authenticate_client(conn, server_private_key, users):
    ...
    Client sends:
      encrypted username
      encrypted password
    Each as a separate framed RSA-encrypted message.
    """
...
def handle_send_email(conn, username, sym_key, users):
    send_encrypted(conn, sym_key, "Send the email")

    email_text = recv_encrypted(conn, sym_key)
    ...
    try:
        ...
        if email_info["from"] != username:
            send_encrypted(conn, sym_key, "Rejected: sender mismatch.")
            return True

        for recipient in email_info["to"]:
            if recipient not in users:
                send_encrypted(conn, sym_key, "Rejected: unknown recipient " + recipient)
                return True
        ...
        send_encrypted(conn, sym_key, "Email stored successfully.")
        return True

    except Exception as e:
        send_encrypted(conn, sym_key, "Rejected: " + str(e))
        return True


def handle_display_inbox(conn, username, sym_key):
    inbox_text = build_inbox_listing(username)
    send_encrypted(conn, sym_key, inbox_text)

    ack = recv_encrypted(conn, sym_key)
    if ack is None:
        return False
    return True


def handle_display_email(conn, username, sym_key):
    send_encrypted(conn, sym_key, "the server request email index")

    enc_index = recv_encrypted(conn, sym_key)
    ...
    try:
        ...
        send_encrypted(conn, sym_key, "Invalid email index.")
        return True
    ...
    send_encrypted(conn, sym_key, email_text)
    return True
...
def handle_client(conn, addr, server_private_key, users):
    ...
    try:
        ...
        while True:
            send_encrypted(conn, sym_key, MENU)
            choice = recv_encrypted(conn, sym_key)
            ...
            if choice == "1":
                if not handle_send_email(conn, username, sym_key, users):
                    break
            elif choice == "2":
                if not handle_display_inbox(conn, username, sym_key):
                    break
            elif choice == "3":
                if not handle_display_email(conn, username, sym_key):
                    break
            else:
